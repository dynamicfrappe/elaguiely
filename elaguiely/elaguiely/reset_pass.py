...
from elaguiely.auth_api import login as make_login
...
def ask_for_forget_password(**kwargs):
	...
	#return respose_info(status=200,name=new_reset_pass_doc.name,code=new_reset_pass_doc.name)
	return
...
def validate_reset_request(**kwargs):
	try:
		...
		#return respose_info(status=200,code=code,msg='Valid Code')
		return 
	except Exception as er:
		frappe.local.response['http_status_code'] = 400
		frappe.response["message"] =_(f"{er}")
		# return respose_info(status=400,msg='Not Valid Code')
...
def reset_password(**kwargs):
	...
	rest_pass_doc = frappe.get_doc('Reset Password',code)
	if not rest_pass_doc or (password != repeat_password):
		frappe.local.response['http_status_code'] = 400
		frappe.response["message"] =_("Wrong Code or Password Not Match")
		# return respose_info(status=400,msg='Wrong Code or Password Not Match')
		return 
	try:
		...
		return make_login(usr=user.name,pwd=password)
	except Exception as er:
		frappe.local.response['http_status_code'] = 400
		...
		# return respose_info(status=400,msg='Password Not Valid')
		return
